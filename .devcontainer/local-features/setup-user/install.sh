...
# For the universal image, oryx build tool installs the detected platforms in /home/jfalco/*. Hence, linking current platforms to the /home/jfalco/ path and adding it to the PATH.
# This ensures that whatever platfornm versions oryx detects and installs are set as root.
NODE_PATH="/home/jfalco/.nodejs/current"
mkdir -p /home/jfalco/.nodejs
ln -snf /usr/local/share/nvm/current $NODE_PATH

PHP_PATH="/home/jfalco/.php/current"
mkdir -p /home/jfalco/.php
ln -snf /usr/local/php/current $PHP_PATH

PYTHON_PATH="/home/jfalco/.python/current"
mkdir -p /home/jfalco/.python
ln -snf /usr/local/python/current $PYTHON_PATH
ln -snf /usr/local/python /opt/python

JAVA_PATH="/home/jfalco/.java/current"
mkdir -p /home/jfalco/.java
ln -snf /usr/local/sdkman/candidates/java/current $JAVA_PATH

RUBY_PATH="/home/jfalco/.ruby/current"
mkdir -p /home/jfalco/.ruby
ln -snf /usr/local/rvm/rubies/default $RUBY_PATH

DOTNET_PATH="/home/jfalco/.dotnet"
...
MAVEN_PATH="/home/jfalco/.maven/current"
mkdir -p /home/jfalco/.maven
ln -snf /usr/local/sdkman/candidates/maven/current $MAVEN_PATH

HUGO_ROOT="/home/jfalco/.hugo/current"
mkdir -p /home/jfalco/.hugo
ln -snf /usr/local/hugo $HUGO_ROOT

HOME_DIR="/home/jfalco/"
chown -R jfalco:jfalco ${HOME_DIR}
chmod -R g+r+w "${HOME_DIR}"
find "${HOME_DIR}" -type d | xargs -n 1 chmod g+s

OPT_DIR="/opt/"
chown -R jfalco:oryx ${OPT_DIR}
chmod -R g+r+w "${OPT_DIR}"
find "${OPT_DIR}" -type d | xargs -n 1 chmod g+s

echo "Defaults secure_path=\"${DOTNET_PATH}:${NODE_PATH}/bin:${PHP_PATH}/bin:${PYTHON_PATH}/bin:${JAVA_PATH}/bin:${RUBY_PATH}/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/local/bin:/usr/local/share:/home/jfalco/.local/bin:${PATH}\"" >> /etc/sudoers.d/$USERNAME
